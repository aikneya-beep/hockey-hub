...
def _memory_gallery(item: dict) -> str:
    ...
    gallery_id=f'memory-{int(item["id"])}'
    return '<div class="memory-photos" data-gallery="'+gallery_id+'">'+"".join(
        f'''<figure style="--photo-bg:url('/my-hockey/memory/photo/{int(p["id"])}')"><button type="button" class="memory-photo-link" data-gallery="{gallery_id}" data-index="{idx}" data-src="/my-hockey/memory/photo/{int(p["id"])}" aria-label="Открыть фотографию {idx+1} из {len(photos)}"><img loading="lazy" src="/my-hockey/memory/photo/{int(p["id"])}" alt=""></button></figure>'''
...
def _memory_edit(item: dict) -> str:
    photos=item.get("photos") or []
    current="".join(
        f'''<label class="edit-photo" title="Удалить фотографию"><input class="photo-delete-input" type="checkbox" name="delete_photo_ids" value="{int(p["id"])}"><img src="/my-hockey/memory/photo/{int(p["id"])}"><span class="photo-delete-x" aria-hidden="true">×</span></label>'''
        for p in photos
...
(()=>{{
  ...
  document.addEventListener('click',e=>{{
    const button=e.target.closest('.memory-photo-link');
    if(button){{openFrom(button);return;}}
  }});
...
def render_home_v56()->str:
    ...
    for score in soup.select(".score-value"):
        text=score.get_text(" ",strip=True)
        classes=list(score.get("class") or [])
        classes.append("score-secret" if any(ch.isdigit() for ch in text) else "score-empty")
        score["class"]=classes
    ...
    style=soup.find("style")
    if style: style.append(r"""
.home-personal-grid{display:grid;grid-template-columns:1.15fr .85fr;gap:8px}.home-personal-item{display:block;padding:10px 11px;background:#090f15;border:1px solid #263443;border-radius:10px;text-decoration:none}.home-personal-item.primary{box-shadow:inset 2px 0 #2a86d9}.home-personal-item span{display:block;color:#64778c;font-size:7px;letter-spacing:.08em;margin-bottom:5px}.home-personal-item b{display:block;color:#d9e0e8;font-size:9px;line-height:1.35;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.home-personal-item em{display:block;color:#637386;font-size:7px;font-style:normal;margin-top:4px}.home-personal-item.counter strong{font-size:15px}.home-personal-item.counter i{font-size:8px;font-style:normal;color:#76879a}body.hide-scores .score-value{filter:none!important}
body.hide-scores .score-value.score-secret{position:relative;display:inline-block;width:3.4em;overflow:hidden;white-space:nowrap;color:transparent!important;text-shadow:none!important;user-select:none}
body.hide-scores .score-value.score-secret:after{content:"—:—";position:absolute;inset:0;color:#cbd3dd;text-align:center}
body.hide-scores .score-value.score-empty{filter:none!important}
